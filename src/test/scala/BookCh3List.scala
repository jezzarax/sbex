...
class BookCh3List extends FlatSpec with Matchers {
  ...
  object List {
    ...
    def hasSubsequence[A](sup: List[A], sub: List[A]): Boolean = {
      ...
      def startsWith[A](hay: List[A], needle: List[A]):Boolean = {
  ...
  "Ex3.24" should "implement hasSubsequence" in {
    ...
    List.hasSubsequence(List(2), List(2)) should be (true)
